...
(define-public (register-asset (asset <sip010>) (name (string-ascii 32)) (decimals uint))
  (begin
    ...
    (map-set asset-registry (contract-of asset) {name: name, decimals: decimals, active: true})
    (print {event: "register-asset", asset: (contract-of asset), name: name, decimals: decimals, by: tx-sender})
    (ok true)))
...
(define-public (deposit-sip010 (asset <sip010>) (amount uint))
  (let ((asset-contract (contract-of asset))
    ...
    (print {event: "deposit-sip010", user: tx-sender, asset: asset-contract, amount: amount, new-balance: (+ current-balance amount)})
    (ok amount)))
...
(define-public (withdraw-sip010 (asset <sip010>) (amount uint))
  (let ((asset-contract (contract-of asset))
    ...
    (print {event: "withdraw-sip010", user: tx-sender, asset: asset-contract, amount: amount, new-balance: (- balance amount)})
